...
VL_INLINE_OPT void Vcounter___024root___nba_sequent__TOP__0(Vcounter___024root* vlSelf) {
    ...
    // Body
    if (vlSelfRef.rst_n) {
        if (vlSelfRef.on_switch) {
            if (vlSelfRef.count_en) {
                vlSelfRef.count = (0xfffU & ((IData)(1U) 
...
#endif
            VL_FATAL_MT("RTL/counter.v", 1, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("RTL/counter.v", 1, "", "Active region did not converge.");
            }
...
void Vcounter___024root___eval_debug_assertions(Vcounter___024root* vlSelf) {
    ...
    if (VL_UNLIKELY((vlSelfRef.on_switch & 0xfeU))) {
        Verilated::overWidthError("on_switch");}
    if (VL_UNLIKELY((vlSelfRef.count_en & 0xfeU))) {
