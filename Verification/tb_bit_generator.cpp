...
int main(int argc, char** argv) {
    ...
    std::ofstream csv_file("report/seed_ver_report_for_new_eqn.csv");
    if (!csv_file.is_open()) {
    ...
    csv_file.close();
    delete top;
    std::cout << "RTL Verification completed! Report saved to 'report/seed_ver_report_for_new_eqn.csv'" << std::endl;
    return 0;
}
...
// Replicates Seed_Memory serial shift behavior (LSB first → bit reversal)
uint16_t bit_reverse_12(int val) {
    uint16_t result = 0;
    for (int i = 0; i < 12; ++i)
        result = (result << 1) | ((val >> i) & 1);
...
// Auto-correlation of a bitstream across lags 1 to max_lag
// Bidirectional Auto-correlation of a bitstream across lags -max_lag to max_lag
double calculate_autocorrelation(const std::vector<int>& bits, int max_lag) {
    int N = bits.size();
    if (N == 0) return 1.0;
    ...
    // Check both forward (lag > 0) and backward/opposite (lag < 0) directions
    for (int lag = -max_lag; lag <= max_lag; ++lag) {
        if (lag == 0) continue; // Skip lag 0 (self-correlation is always 1)
        ...
        for (int t = 0; t < N; ++t) {
            int t_lagged = t + lag;
            if (t_lagged >= 0 && t_lagged < N) {
                cov += (bits[t] - mean) * (bits[t_lagged] - mean);
                count++;
            }
        }
...
int main(int argc, char** argv) {
    ...
    // Fixed reference values aligned with expected thresholds
    std::vector<uint16_t> B_refs(32);
    for (int r = 0; r < 32; ++r) {
        B_refs[r] = r * 128; // Standardized intervals for 4096 scale
    }
    ...
    std::cout << "Simulating all 4096 seeds with corrected alignment..." << std::endl;
    ...
    csv_file.close();
    delete top;
    std::cout << "RTL Verification completed successfully!" << std::endl;
    return 0;
}
*/