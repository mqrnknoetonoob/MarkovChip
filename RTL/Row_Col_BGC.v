module Row_Col_BGC (
    ...
    wire [11:0] lfsr_row_out;
    wire [11:0] lfsr_col_out;
    wire comp_row_out;
    wire comp_col_out;
    wire multiplication_result;

    // --- Column MUX & Mode Selection ---
    // If output_ready is 1, we parallel load (2'b10) the prev_step_output value.
    // Otherwise, we follow the top FSM's mode_col.
    wire [1:0] col_reg_mode  = output_ready ? 2'b10 : mode_col;
    wire [11:0] col_data_in  = output_ready ? prev_step_output : 12'd0;

    // --- Row Mode Selection ---
    // If output_ready is 1, row must hold its value (2'b00) since only the
    // column reloads with prev_step_output at a step transition.
    // Otherwise, follow the top FSM's mode_row.
