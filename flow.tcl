...
proc run_pdn_bridge_patch_step {args} {
    # Design-scoped, opt-in hook: only does anything if the current design
    # has its own designs/<DESIGN>/hooks/post_routing_patch.py. For every
    # other design (no such file present) this step is a silent no-op.
    set hook_script "$::env(DESIGN_DIR)/hooks/post_routing_patch.py"
    if { [file exists $hook_script] } {
        puts_info "Running design-local post-routing patch hook: $hook_script"
        set def_path $::env(CURRENT_DEF)
        set patched_path "${def_path}.patched"
        exec python3 $hook_script $def_path $patched_path
        file copy -force $patched_path $def_path
        puts_info "Patch applied in place to $def_path"

        # IMPORTANT: downstream steps default to reading the saved .odb
        # binary snapshot (IO_READ_DEF=0 in this config), NOT re-parsing
        # the .def text file -- so a text-only patch to $def_path is
        # otherwise invisible to every step after this one. flow.tcl
        # itself runs in plain tclsh and does NOT have OpenROAD commands
        # (read_lef/read_def/write_db) available directly -- those only
        # exist inside an actual openroad process. So this refresh is
        # done via a real OpenROAD subprocess, exec'd the same way
        # OpenLane's own built-in post_run.py hook mechanism already
        # does it (see run_post_run_hooks in this same file).
        if { [info exists ::env(CURRENT_ODB)] } {
            set refresh_script "$::env(DESIGN_DIR)/hooks/refresh_odb_from_def.tcl"
            if { [file exists $refresh_script] } {
                puts_info "Refreshing ODB from patched DEF: $::env(CURRENT_ODB)"
                exec $::env(OPENROAD_BIN) -exit -no_init $refresh_script
                puts_info "ODB refreshed: $::env(CURRENT_ODB)"
            } else {
                puts_warn "Expected ODB-refresh script not found at $refresh_script \
                    -- ODB was NOT refreshed. Downstream steps reading ODB (not DEF) \
                    will not see this patch."
            }
        } else {
            puts_warn "CURRENT_ODB not set -- could not refresh the ODB snapshot. \
                Downstream steps reading ODB (not DEF) may not see this patch. \
                Check IO_READ_DEF / CURRENT_ODB env handling for this flow version."
        }
    } else {
        puts_verbose "No post-routing patch hook found at $hook_script, skipping."
    }
} 
...
proc run_non_interactive_mode {args} {
    ...
    set steps [dict create \
        ...
        "pdn_bridge_patch" "run_pdn_bridge_patch_step" \
